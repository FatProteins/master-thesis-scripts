...
filename = "data/5900-kv_pairs_2023-11-01T19-19-40-follower_1_crash_5900_client.csv"
# if len(sys.argv) != 2:
...
ax.axvline(25, ymax=0.81, color='r', linestyle='--', zorder=1)
ax.text(16, 35000, 'Follower\nCrash', color='r')
ax.axvline(38, ymax=0.92, color='b', linestyle='--', zorder=1)
ax.text(39, 33000, 'Follower\nRejoins\nCluster', color='b')
...
ax.set_ylim(0, ax.get_ylim()[1])
plt.savefig(f"follower_1_crash_{num_clients}_client_{random.Random().randint(0, 10000)}.pdf", bbox_inches='tight', pad_inches=0.05)
plt.show()
plt.clf()
